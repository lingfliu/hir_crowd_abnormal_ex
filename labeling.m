...
video30 = zeros(1,2938);
video30(1,1:1086)=4;
video30(1,1087:end)=0;
behavelabels{30} = video30;

% video31 = zeros(1,1200);
video31 = zeros(1,1199);
video31(1,1:830)=4;
video31(1,831:end)=2;
behavelabels{31} = video31;

n_frame = 0;
for m = 1:length(behavelabels)
    n_frame = n_frame + length(behavelabels{m});
end

label_collapsed = zeros(n_frame,3);

idx = 0;
for m = 1:length(behavelabels)
    label = behavelabels{m};
    for n = 1:length(label)
        idx = idx + 1;
        label_collapsed(idx,:) = [m,n, label(n)];
    end
end
save("label_collapsed", 'label_collapsed');
